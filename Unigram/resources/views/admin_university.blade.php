    ...
    <div class="search-box">
      ...
      <table class="leaderboard-table">
        ...
        <tbody>
          ...
          @foreach ($universities as $user)
           <tr>
       
            <td>{{ $user->id }}</td>
            <td><form action="{{ URL::to('/') }}/admin_university_view" method="POST">
                @csrf
                <input name='university_id' value="{{ $user->id }}" hidden/>  
                <button type="submit" id="add_to_leaderboard_btn">{{ $user->username }}
            </button></a></td>
            <td>{{ $user->email }}</td>
            <td>{{ $user->mobile }}</td>
